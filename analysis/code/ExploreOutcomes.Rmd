...
PlotOutcomeLines <- function(outcome, dt=data, city_list=NULL, window=1300:1918){
  ...
  plot <- dt %>% 
    ...
    drop_na({{outcome}}) %>% 
    ggplot(aes(x=year, colour=city_id))+
...
wage_plot <- PlotOutcomeLines(real_wage)
wratio_plot <- PlotOutcomeLines(welfare_ratio)
const_plot <- PlotOutcomeLines(construction, city_list = both)
...
TopConstCities <- function(data, howmany=8, by="obs", window=1300:1918){
  cities_life <- data %>% 
    filter(year %in% window) %>% 
    group_by(city_id) %>% 
    summarise(
  ...
  return(top_ids$city_id[1:howmany])
}

city_list <- TopConstCities(data, howmany=5)

PlotOutcomeLines(construction, city_list = city_list)
```
