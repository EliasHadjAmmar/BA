...
There are no duplicate rows, but there are 3708 observations with duplicate city-year keys. These observations make up 0.3% of the data set. I can probably drop them? Unless they are very different from the rest for some reason. *0.337% is suspiciously close to the share of city-years with extinctions.*

```{r get obs with dupe keys}

# if there are multiple observations for a city-year key, number them
data_count_dupes <- data %>% 
  group_by(city_id, year) %>% 
  mutate(count_dupe = row_number()) %>% 
  ungroup()

# show that all observations with duplicate keys have terr_id A1341
data_count_dupes %>% filter(count_dupe != 1) %>% ggplot() + geom_bar(aes(x=terr_id))

# show that exactly half of all A1341 observations are dupes
data_count_dupes %>% 
  filter(terr_id == "A1341") %>% 
  mutate(count_dupe = factor(count_dupe)) %>% 
  ggplot(aes(x=count_dupe)) + 
  geom_bar() +
  geom_text(stat='count', aes(label=after_stat(count)), vjust=0)

# suspicion: the only thing that varies for dupes is `X` (auto-generated while joining)
# and `count_diff` (only calculated for the first instance of a city-year observation).
data %>% filter(terr_id == "A1341") %>% select(-X, -count_diff) %>% distinct() %>% count()

```

Conclusion: for some reason, all observations with terr_id "A1341" got duplicated somewhere along the way. I *could* just drop all the dupes, because the `count_diff` is always with the first version:
...
data_count_dupes %>% 
  ...
  mutate(count_dupe = factor(count_dupe)) %>% 
  ggplot(aes(x=count_dupe)) + 
  geom_boxplot(aes(y=count_diff))
```

But because this isn't actually the final version of the data, and this might happen again, it's probably prudent to check where the duplication occurs. I do this in a separate Rmd.
...
terrs <- data %>% 
  ...
  select(terr_id, year, territory, count_cities, final_year, extinction)
```
...
all city-years in a lineage-year, so `count_cities` and `extinction`.
