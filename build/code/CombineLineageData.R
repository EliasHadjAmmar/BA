...
Main <- function(){
  lineages <- read_csv("build/temp/terr_sizes.csv", show_col_types = F)
  terr_codes <- read_csv("build/input/territory_codes.csv", show_col_types = F)
  extinctions <- read_csv("build/output/extinctions.csv", show_col_types = F) |> 
  ...
  lineages %>% write_csv("build/output/lineages.csv")
}
