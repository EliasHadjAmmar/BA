...
\begin{center}
    {\centering\Large{{Conflict-Driven Evolution and Urban Development:\\
...
By contrast, my story is not about population dynamics but institutions (although it is compatible with Malthusianism). A longstanding tradition in economics emphasises the role of institutional factors in long-run growth (e.g. \cite{north1970}, \cite{delong1993}, \cite*{ajr2001}). From this vast literature one theme may be especially relevant here: the institution of fiscal capacity, or the state's ability to collect taxes. \cite{tilly1985} argues that the main reason for states to collect taxes is to be able to wage war. In other words, fiscal capacity and state power are tightly linked --- so if conflict selects for state power, then it also selects for fiscal capacity. \cite{gennaioli2015} and \cite*{cantoni2023} show that conflict drove state consolidation, with the latter set of authors using the same data as this paper. \cite{dincecco2012} show that fiscal capacity had a positive effect on growth.

\cite{diamond1997} and \cite{landes1969, landes2006} have argued for the economic benefits of political fragmentation and state competition. \cite{diamond1997} asserts that the geographic remoteness of China deprived it of the benefits of state competition that European states enjoyed throughout their history. \cite{landes2006} makes a similar argument, pointing out how the totalitarianism and bureaucratism that stifled the spread of innovation in medieval China would not have survived long in a more fragmented state system. Geography and fragmentation also play central roles in \cite{cervellati2022}'s theory of European state history.
...
\begin{figure}[ht]
    \centering
    \includegraphics[scale = 0.3]{paper/output/regressions/SW22_replication_50y.png}
...
The only subset of construction for which the difference in treatment effects is indeed significant is state construction --- palaces, castles, military and administrative buildings. We can see in Column (2) that these are 5 percentage points \textit{less} likely to be built in a city after it was conquered; the effect is significant at the 5\% level. It is hard to explain this fact with state power; however, keeping in mind that this is the effect of conquests relative to successions, it could be that rulers who inherit a city feel a bigger desire to impress and make their presence known to the townspeople than someone who just marched an army into it.
...
While the research question may not seem hugely relevant for the present-day Western world, I want to conclude by talking, briefly, about the Democratic Republic of the Congo (DRC). This African state has a weak central government and large parts of its territory are controlled by independent armed groups that are in many ways like small proto-states \citep[p. 34]{sierra2020}. Recent research has studied state formation \citep{sierra2020}, taxation and state capacity \citep{balan2022}, and conflict \citep{henn2021} in the setting of the DRC. One could argue that we are witnessing economic history "in real time" there.\footnote
{
This thought is not my own; I got it from Davide Cantoni.
} 
In this sense a better understanding of how the European state system evolved may not only deepen our knowledge of the origins of institutions --- it could also inform our perspective, and those of policy makers, on regions of the world that do not have a stable state system at present.
