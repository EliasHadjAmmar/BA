...
\cite{schoenholzer2022} find a short-run cost of switching (presumably due to conflict) and a long-run benefit. They get the short-term result by looking at cities that only switched states briefly before switching back, and the long-run result by comparing that to cities that permanently stayed with their new territory. I operate in the same framework and I borrow their notation: the variable $S_{it}$ is the number of times a city switched states in period $t$. By contrast, the variable $e^{NewState}_i$ denotes the first period in which city $i$ belongs to a different state than the previous. Intuitively, in periods with $S_{it} > 0$ a city suffers short-run effects of contemporary switching, while in periods following $e^{NewState}_i$ it will be under new rule and therefore experience long-run effects of switching. 
...
where the outcome $construction_{it}$ is an indicator that equals 1 if any construction activity was recorded in city $i$ and period $t$, the variable $S_{it}$ is the number of times a city switched states in period $t$, the variable $e^{NewState}_i$ denotes the first period in which city $i$ belongs to a different state than the previous, and $\alpha_i$ and $\delta_t$ denote city and period fixed effects. The coefficients of interest are the dynamic effects $\pi_\tau$ of switching to a new state permanently.

Since the outcome is binary, equation \ref{eq:sw22} represents a linear probability model: we model $construction_{it}$ as a random variable that follows a Bernoulli distribution with a probability parameter $p$ that is conditional on the right-hand side variables. This way we can interpret the values of coefficients as differences in the probability of recorded construction activity in a given period. 

The coefficient $\pi_\tau$ for given $\tau$ is the difference in the probability of construction between two cities $i$ and $j$ with identical time-invariant characteristics, in the same period, who undergo the same amount of switching turmoil in this period. The only difference is that one city underwent a lasting switch to another state $\tau$ periods ago, while the other did not. 

% Since comparisons between cities at different \tau are problematic, we also use the Sun and Abraham estimator (as do SW22).

Remember, we expect this difference to stem from the fact that switching states generally means switching to a higher-quality state (due to selection). As such, even though it would be possible in principle to include state fixed effects, it is not a good idea here: state fixed effects would absorb the variation in state quality whose effects we want to capture in $\pi_\tau$. 
...
\label{eq:baseline}
    ...
    \mathbf{1}(t \ge e^{NewState}_i) T_i \theta + \varepsilon_{it}
\end{equation}
