...
Econometrics come on top of this basic assumption about the interpretation of the independent variable. The parallel trends assumption is: Had a conquered city $i$ counterfactually been acquired by succession rather than conquest, it would have trended the same as the other cities acquired by succession. 

The first implication of that is that cities did not select into treatment based on uncontrolled-for characteristics that . 
There are no uncontrolled-for characteristics that made a city more or less likely to be conquered and also had an effect on construction activity. Note that I control for all time-invariant characteristics of cities in the city fixed effect.

The second implication is there is nothing else happening at the same time as the conquest that affects conquered vs. inherited cities differently.

If conquests had an effect on construction activity not just through state power but also through the number of cities that also switched - maybe switching to a new state alone is better than switching with all cities of an entire territory.
