...
These arguments could imply that the difference between conquering and inheriting a city does not really capture state power differentials --- that my measure does not actually contain much information about the independent variable. If conquests are at best a noisy measure of state power, we essentially have measurement error in the independent variable. Accordingly, we should expect attenuation bias to push the result towards zero. The estimate for $\theta_{conquest}$ has the expected positive sign in my baseline regression --- it just lacks statistical significance. If we could regress differences in construction trends directly on observed differences in state power, we would sidestep this measurement error and likely find larger and significant effects.

On top of this basic assumption about the interpretation of the independent variable come econometric assumptions. BLANK. External validity
